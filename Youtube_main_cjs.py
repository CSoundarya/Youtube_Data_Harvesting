...
st.set_page_config(page_title= "Youtube Data Harvesting and Warehousing | By Name",
                   page_icon= icon,
                   layout= "wide",
                   initial_sidebar_state= "expanded",
                   menu_items={'About': """# This app is created by Name """})
...
client = pymongo.MongoClient("id")
db = client.youtube_data

# CONNECTING WITH MYSQL DATABASE
mydb = sql.connect(host="localhost",user="root",password="",database= "youtube_data")
        ...
        if st.button("Upload to MongoDB"):
            with st.spinner('Please Wait for it...'):
                ...
                if len(comm_details) <=0:
                    comm_details = {'Channel_name':' null','Channel_id':'null ','Video_id':' null','Title':' null','Tags':' null','Thumbnail':' null','Description':' null','Published_date':' null','Duration':' null','Views':' null','Likes':' null','Comments':' null','Favorite_count':'null ','Definition':' null','Caption_status':' null'}
                    collections3.insert_one(comm_details)
                else:
    ...
    with tab2:     
        ...
        def insert_into_channels():
                ...
                for i in collections.find({"Channel_name" : user_inp},{'_id' : 0}):
                    dis_list=i['Description'].split(' ')
                    if len(dis_list) > 15:
                        dis_list = dis_list[:15]
                        sentence = ' '.join(dis_list)
...
if selected == "View":
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        mycursor.execute("""SELECT DISTINCT title AS Video_Title, channel_name AS Channel_Name
                            FROM videos
        ...
        mycursor.execute("""SELECT DISTINCT channel_name AS Channel_Name, total_videos AS Total_Videos
                            FROM channels
        ...
        mycursor.execute("""SELECT DISTINCT channel_name AS Channel_Name,
                            AVG(
        CONVERT(
            SUBSTRING_INDEX(SUBSTRING_INDEX(duration, 'H', -1), 'M', 1) * 60 +
            SUBSTRING_INDEX(SUBSTRING_INDEX(duration, 'M', -1), 'S', 1),
            UNSIGNED INTEGER
        )
    ) AS average_minutes
                            FROM videos
